...
export default function TwitterDownloader() {
  ...
  const simulateProcessingStages = () => {
    const stages = [
      { stage: 'validating', message: 'Validating Twitter/X URL...', percent: 15 },
      { stage: 'connecting', message: 'Connecting to Twitter/X servers...', percent: 35 },
      { stage: 'extracting', message: 'Extracting video information...', percent: 60 },
      { stage: 'analyzing', message: 'Finding best quality version...', percent: 80 },
  ...
  const validateTwitterURL = (url: string): boolean => {
    const twitterPatterns = [
      ...
      /(?:https?:\/\/)?(?:www\.|mobile\.)?x\.com\/[\w]+\/status\/\d+/,
      /(?:https?:\/\/)?t\.co\/[\w]+/,
    ];

    return twitterPatterns.some(pattern => pattern.test(url.toLowerCase()));
  };
  ...
  const handleSubmit = async (e: React.MouseEvent | React.KeyboardEvent) => {
    ...
    try {
      ...
      setVideoInfo(data);
      setProcessingStatus({ stage: 'complete', message: 'Twitter video ready!', percent: 100 });
      ...
      <div
        className={`h-2 rounded-full transition-all duration-300 ${progress.status === 'error' ? 'bg-red-500' :
            progress.status === 'completed' || progress.status === 'downloaded' ? 'bg-emerald-500' :
              progress.status.includes('retrying') ? 'bg-amber-500' : 'bg-blue-500'
          }`}
    ...
    <div className="max-w-4xl mx-auto mb-6 sm:mb-8 px-4">
      <div className="bg-slate-800/80 border border-blue-400/30 rounded-xl p-4 sm:p-6 text-center backdrop-blur-sm">
        ...
        <div className="w-full bg-slate-700 rounded-full h-2 sm:h-3 mb-2">
          <div
            className={`h-2 sm:h-3 rounded-full transition-all duration-500 ${status.stage === 'complete' ? 'bg-emerald-500' : 'bg-gradient-to-r from-blue-500 to-sky-500'
              }`}
        ...
        <div className="flex justify-between text-xs sm:text-sm text-gray-300">
          <span>Processing Twitter/X video...</span>
  ...
  return (
    <div className="min-h-screen pt-4 sm:pt-20 bg-gradient-to-br from-slate-900 via-gray-900 to-zinc-900">
      <div className="absolute inset-0 bg-gradient-to-br from-blue-600/10 via-sky-600/5 to-cyan-600/10" />
      <div className="relative z-10 container mx-auto px-4 py-4 sm:py-8">

        {/* Mobile Responsive Header */}
        <div className="text-center mb-6 sm:mb-12">
          <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold text-white mb-3 sm:mb-4 bg-gradient-to-r from-blue-400 via-sky-400 to-cyan-400 bg-clip-text text-transparent">
            Twitter/X Video Downloader
          </h1>
          <p className="text-sm sm:text-lg md:text-xl text-gray-300 max-w-2xl mx-auto">
            Download Twitter and X videos without watermark in HD quality
          </p>
          <div className="flex justify-center items-center gap-1 sm:gap-2 mt-3 sm:mt-4">
            ...
            <span className="text-blue-300 font-semibold text-sm sm:text-base">High Quality Downloads</span>
            <Star className="h-4 w-4 sm:h-5 sm:w-5 text-blue-400" />
          </div>
        </div>
          ...
          <div className="relative">
            ...
            <div className="hidden md:flex rounded-2xl overflow-hidden shadow-2xl bg-slate-800/80 backdrop-blur-lg border border-slate-700/50">
              <input
                ...
                value={url}
                onChange={(e) => setUrl(e.target.value)}
                placeholder="Paste Twitter/X URL here... (e.g., https://twitter.com/user/status/123)"
              ...
              <button
                onClick={handleSubmit}
                disabled={loading || !url.trim()}
                className="px-8 py-4 bg-gradient-to-r from-blue-500 to-sky-500 text-white font-semibold hover:from-blue-600 hover:to-sky-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center gap-2"
              >
                {loading ? (
              ...
              <div className="p-4 sm:p-6 md:p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
                  <div className="md:col-span-1">
                    <div className="relative">
                      ...
                      <div className="absolute bottom-2 left-2 bg-gradient-to-r from-blue-500 to-sky-500 text-white px-2 py-1 rounded-md text-xs font-semibold">
                        HD QUALITY
                      </div>
                    </div>
                  </div>
                  <div className="md:col-span-2">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6">
                      ...
                      <div className="flex items-center gap-1 sm:gap-2 text-gray-300">
                        ...
                        <span className="text-xs sm:text-sm">{formatLikeCount(videoInfo.repost_count)} retweets</span>
                      </div>
                      <div className="flex items-center gap-1 sm:gap-2 text-gray-300">
                ...
                <div className="space-y-4 sm:space-y-6">
                  {downloadMode === 'video' ? (
                    <div>
                      ...
                      {videoInfo.formats?.video_formats && videoInfo.formats.video_formats.length > 0 ? (
                              ...
                              <div key={index} className="bg-slate-700/50 rounded-xl p-3 sm:p-4 border border-blue-500/20">
                                <div className="flex justify-between items-start mb-2">
                                  <div className="flex-1">
                                    ...
                                    <div className="text-xs text-blue-400 mt-1 flex items-center gap-1 flex-wrap">
                                ...
                                <button
                                  ...
                                  className="w-full bg-gradient-to-r from-blue-500 to-sky-500 hover:from-blue-600 hover:to-sky-600 text-white font-medium py-2 sm:py-2 px-3 sm:px-4 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm sm:text-base"
                                >
                        ...
                        <div className="text-center py-6 sm:py-8">
                          <p className="text-gray-400 text-base sm:text-lg">No video formats available for this Twitter/X video.</p>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div>
                      <div className="flex items-center gap-2 mb-3 sm:mb-4">
                        <Music className="h-4 w-4 sm:h-5 sm:w-5 text-sky-400" />
                      ...
                      {videoInfo.formats?.audio_formats && videoInfo.formats.audio_formats.length > 0 ? (
                        <div className="grid gap-3 sm:gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                          {videoInfo.formats.audio_formats.map((format, index) => {
                            ...
                            return (
                              <div key={index} className="bg-slate-700/50 rounded-xl p-3 sm:p-4 border border-sky-500/20">
                                <div className="flex justify-between items-start mb-2">
                                  <div className="flex-1">
                                    <span className="text-white font-medium text-sm sm:text-base">
                                      {format.quality} (MP3)
                                ...
                                <button
                                  onClick={() => handleDownload(format)}
                                  disabled={!!progress && !['error', 'downloaded'].includes(progress.status)}
                                  className="w-full bg-gradient-to-r from-sky-500 to-cyan-500 hover:from-sky-600 hover:to-cyan-600 text-white font-medium py-2 sm:py-2 px-3 sm:px-4 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm sm:text-base"
                                >
                                  <Download className="h-3 w-3 sm:h-4 sm:w-4" />
                                  {getButtonText(progress)}
                                </button>
                        ...
                        <div className="text-center py-6 sm:py-8">
                          <p className="text-gray-400 text-base sm:text-lg">No audio formats available for this Twitter/X video.</p>
                        </div>
                      )}
                    </div>
                  )}
        ...
        <div className="text-center mt-8 sm:mt-12 text-gray-400 px-4 sm:px-0">
          <p className="text-sm sm:text-base">© 2025 Twitter/X Video Downloader. Download videos in HD quality.</p>
          <p className="text-xs sm:text-sm mt-2">Supports all Twitter/X video formats • Fast downloads • No registration required</p>
        </div>
      </div>
    </div>
  );
