...
import { Search, Download, Clock, Eye, User, Music, Loader, CheckCircle, AlertTriangle, Volume2, Play, Headphones, ExternalLink } from 'lucide-react';
...
export default function YouTubeMp3Downloader() {
  ...
  const simulateProcessingStages = () => {
    const stages = [
      { stage: 'validating', message: 'Validating YouTube URL...', percent: 15 },
      { stage: 'connecting', message: 'Connecting to YouTube...', percent: 35 },
      { stage: 'extracting', message: 'Extracting video information...', percent: 60 },
      { stage: 'analyzing', message: 'Analyzing audio quality options...', percent: 80 },
      { stage: 'finalizing', message: 'Preparing MP3 download...', percent: 95 },
  ...
  const handleSubmit = async (e: React.MouseEvent | React.KeyboardEvent) => {
    ...
    if (!validateYouTubeURL(url.trim())) {
      setError('Please provide a valid YouTube URL (videos, shorts, or embeds)');
      return;
    }
  ...
  const handleDownload = async (format: AudioFormat) => {
    ...
    try {
      ...
      console.log('YouTube MP3 download parameters:', downloadParams);
      ...
      <div className="relative z-10 container mx-auto px-4 py-4 sm:py-8">
        ...
        <div className="text-center mb-6 sm:mb-12">
          <div className="flex items-center justify-center gap-2 mb-3 sm:mb-4">
            <Play className="h-8 w-8 sm:h-12 sm:w-12 text-red-500" />
            <h1 className="text-2xl sm:text-4xl md:text-5xl font-bold text-white bg-gradient-to-r from-red-400 via-pink-400 to-orange-400 bg-clip-text text-transparent">
              YouTube to MP3 Converter
            </h1>
          </div>
          <p className="text-sm sm:text-lg md:text-xl text-gray-300 max-w-3xl mx-auto mb-2">
            Convert YouTube videos to high-quality MP3 audio files in multiple bitrates (128-320kbps)
          </p>
          <p className="text-xs sm:text-sm text-gray-400 max-w-2xl mx-auto">
            Free Online YT Audio Downloader
          </p>
          <div className="flex justify-center items-center gap-2 sm:gap-4 mt-3 sm:mt-4 flex-wrap">
            <div className="flex items-center gap-1 sm:gap-2 bg-emerald-500/20 text-emerald-300 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm">
          ...
          <div className="relative">
            ...
            <div className="hidden md:flex rounded-2xl overflow-hidden shadow-2xl bg-slate-800/80 backdrop-blur-lg border border-slate-700/50">
              <input
                ...
                value={url}
                onChange={(e) => setUrl(e.target.value)}
                placeholder="Paste YouTube URL here (videos, shorts, or embeds)..."
                  ...
                  <div className="md:col-span-1">
                    <div className="relative">
                      {audioInfo.thumbnail ? (
                        ...
                        <div className="w-full aspect-video bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center rounded-xl border-2 border-dashed border-slate-600">
                          <div className="text-center">
                            <Headphones className="h-16 w-16 text-red-400 mx-auto mb-3" />
                            <p className="text-slate-300 text-lg font-medium">YouTube Video</p>
                            <p className="text-slate-500 text-sm">Ready to Convert</p>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="absolute bottom-2 left-2 bg-gradient-to-r from-red-500 to-red-600 text-white px-2 py-1 rounded-md text-xs font-semibold flex items-center gap-1">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6">
                      ...
                      <div className="flex items-center gap-1 sm:gap-2 text-gray-300">
                        ...
                        <span className="text-xs sm:text-sm">MP3 Audio</span>
                      </div>
                      {audioInfo.duration > 0 && (
                        <div className="flex items-center gap-1 sm:gap-2 text-gray-300">
                      ...
                      <div className="flex items-center gap-1 sm:gap-2 text-red-300">
                        ...
                        <span className="text-xs sm:text-sm">Multiple Qualities</span>
                      </div>
                      <div className="flex items-center gap-1 sm:gap-2 text-emerald-300 col-span-2 sm:col-span-1">
                        <CheckCircle className="h-3 w-3 sm:h-4 sm:w-4 flex-shrink-0" />
                        <span className="text-xs sm:text-sm font-semibold">Ready to convert</span>
                      </div>
                    </div>
                    <div className="bg-slate-700/30 rounded-lg p-2 sm:p-3">
                      <div className="flex items-center gap-2 mb-2">
                        <ExternalLink className="h-4 w-4 text-red-400" />
                        <span className="text-red-300 text-sm font-medium">YouTube URL:</span>
                      </div>
                ...
                <div className="space-y-4 sm:space-y-6">
                  <div>
                    <div className="flex items-center gap-2 mb-3 sm:mb-4">
                      <Music className="h-4 w-4 sm:h-5 sm:w-5 text-red-400" />
                      <h3 className="text-lg sm:text-xl font-semibold text-white">MP3 Audio Quality Options</h3>
                    ...
                    {audioInfo.formats?.audio_formats && audioInfo.formats.audio_formats.length > 0 ? (
                      <div className="grid gap-3 sm:gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
                        {audioInfo.formats.audio_formats.map((format, index) => {
                          ...
                          return (
                            <div key={index} className="bg-slate-700/50 rounded-xl p-3 sm:p-4 border border-red-500/20 hover:border-red-500/40 transition-all duration-300">
                              ...
                              <button
                                ...
                                className="w-full bg-gradient-to-r from-red-500 to-pink-500 hover:from-red-600 hover:to-pink-600 text-white font-medium py-2 sm:py-2 px-3 sm:px-4 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 text-sm sm:text-base"
                              >
                      ...
                      <div className="text-center py-6 sm:py-8">
                        <p className="text-gray-400 text-base sm:text-lg">No audio formats available for this YouTube video.</p>
        ...
        <div className="text-center mt-8 sm:mt-12 text-gray-400 px-4 sm:px-0">
          <div className="max-w-4xl mx-auto mb-6">
            <h2 className="text-lg sm:text-xl font-semibold text-white mb-3">
              Best Free YouTube to MP3 Converter Online 2025
            </h2>
            <p className="text-xs sm:text-sm text-gray-300 leading-relaxed mb-4">
              Our YouTube to MP3 converter allows you to download and convert YouTube videos to high-quality MP3 audio files.
              Extract audio from YouTube videos, shorts, and music in 128kbps, 192kbps, 256kbps, or 320kbps quality.
              No registration required, unlimited downloads, fast conversion speed, and completely free to use.
            </p>
            <p className="text-xs sm:text-sm text-gray-400 leading-relaxed">
              <strong>How to download YouTube to MP3:</strong> Copy the YouTube video link, paste it above, choose your preferred quality and download.
              Free and fast - no registration required.
            </p>
          </div>

          <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-4 text-xs sm:text-sm">
            <span className="bg-slate-800/50 px-3 py-1 rounded-full">YouTube MP3 Converter</span>
            <span className="bg-slate-800/50 px-3 py-1 rounded-full">YT to MP3</span>
            <span className="bg-slate-800/50 px-3 py-1 rounded-full">YouTube Audio Downloader</span>
            <span className="bg-slate-800/50 px-3 py-1 rounded-full">320kbps Quality</span>
            <span className="bg-slate-800/50 px-3 py-1 rounded-full">Free & No Ads</span>
          </div>

          <p className="text-sm sm:text-base">© 2025 YouTube to MP3 Converter by Snap Save Pro. Convert YouTube videos to high-quality MP3 audio.</p>
          <p className="text-xs sm:text-sm mt-2">Fast conversions • 128-320kbps quality options • Free & No registration</p>
        </div>
      </div>
    </div>
  );
