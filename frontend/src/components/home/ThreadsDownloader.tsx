...
const threadsDownloader: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Validate threads URL - Accept threads.net and threads.com domains
    const threadsRegex = /(threads\.net|threads\.com|www\.threads\.net|www\.threads\.com)/i;
    if (!threadsRegex.test(url.trim())) {
      setError('Please enter a valid Threads URL (threads.net or threads.com)');
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/download`, {
        ...
        body: JSON.stringify({
          ...
          platform: 'threads',
          direct_video_url: (format as any).direct_url, // Pass direct URL for Threads
        }),
      });
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8">
      {/* Header Section with threads Theme (Orange/Yellow) */}
      <div className="text-center mb-12 animate-fade-in">
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-green-400 via-teal-400 to-green-500 bg-clip-text text-transparent animate-gradient">
            {t('title')}
          </h1>
        </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:gap-4">
          <span className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-green-500/10 border border-green-500/30 rounded-full text-blue-300 text-xs sm:text-sm backdrop-blur-sm">
            <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4" />
            {t('hdQuality')}
          </span>
          <span className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-teal-500/10 border border-teal-500/30 rounded-full text-purple-300 text-xs sm:text-sm backdrop-blur-sm">
            <CheckCircle className="w-3 h-3 sm:w-4 sm:h-4" />
            {t('noWatermark')}
          </span>
          <span className="inline-flex items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-green-500/10 border border-green-500/30 rounded-full text-blue-300 text-xs sm:text-sm backdrop-blur-sm">
      ...
      <form onSubmit={handleSubmit} className="mb-8 block md:hidden space-y-3">
        <div className="relative">
          <input
            ...
            className="w-full px-4 sm:px-6 py-3 sm:py-4 bg-slate-800/50 border border-green-500/30 rounded-xl sm:rounded-2xl text-white placeholder-slate-400 focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 backdrop-blur-xl transition-all text-sm sm:text-base pr-12"
            disabled={isLoading}
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                e.preventDefault();
          ...
          <button
            type="button"
            onClick={handlePaste}
            className="absolute right-3 top-1/2 -translate-y-1/2 p-2 hover:bg-green-500/20 rounded-lg transition-colors"
        ...
        <div className="flex gap-3 items-center">
          <div className="relative flex-1">
            <input
              ...
              onChange={(e) => setUrl(e.target.value)}
              placeholder={t('placeholder')}
              className="w-full px-6 py-4 bg-slate-800/50 border border-green-500/30 rounded-2xl text-white placeholder-slate-400 focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 backdrop-blur-xl transition-all pr-12"
              disabled={isLoading}
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  e.preventDefault();
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="px-8 py-4 bg-gradient-to-r from-green-500 to-teal-500 hover:from-green-600 hover:to-teal-600 text-white font-semibold rounded-2xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-green-500/30 hover:shadow-green-500/50 hover:scale-105 whitespace-nowrap"
          >
            {isLoading ? (
      ...
      {videoInfo && (
        <div id="video-info" className="space-y-6 animate-fade-in-up">
          {/* Video Preview Card */}
          <div className="bg-slate-800/50 backdrop-blur-xl rounded-2xl overflow-hidden border border-green-500/20 shadow-2xl">
            <div className="relative">
              <img
            ...
            <div className="p-6">
              ...
              <div className="space-y-3">
                <h3 className="text-lg font-semibold text-white flex items-center gap-2">
                  {downloadMode === 'video' ? <Video className="w-5 h-5 text-green-400" /> : <Music className="w-5 h-5 text-teal-400" />}
                ...
                {(() => {
                  ...
                  if (!currentFormats || currentFormats.length === 0) {
                    return (
                      <div className="p-6 bg-slate-900/50 rounded-xl border border-teal-500/20 text-center">
                  ...
                  return currentFormats.map((format, index) => {
                    ...
                    return (
                      <div
                        ...
                        className="p-4 bg-slate-900/50 rounded-xl border border-green-500/10 hover:border-green-500/30 transition-all"
                        style={{ animationDelay: `${index * 0.1}s` }}
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="px-3 py-1 bg-green-500/20 text-blue-300 rounded-lg text-sm font-semibold">
                                {format.quality}
                              </span>
                          ...
                          <button
                            ...
                            className="px-6 py-2.5 bg-gradient-to-r from-green-500 to-teal-500 hover:from-green-600 hover:to-teal-600 text-white font-semibold rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg shadow-green-500/20 hover:shadow-green-500/40 hover:scale-105 whitespace-nowrap"
                          >
                        ...
                        {progress && (
                          <div className="mt-3 space-y-2">
                            <div className="flex items-center justify-between text-sm">
                              ...
                              <span className="text-green-400 font-semibold">{progress.percent}%</span>
                            </div>
