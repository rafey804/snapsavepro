...
export default function TikTokDownloader() {
  ...
  const handleSubmit = async (e: React.MouseEvent | React.KeyboardEvent) => {
    ...
    // Validate TikTok URL before processing
  ...
  const ProcessingIndicator = ({ status }: { status: ProcessingStatus }) => (
    <div className="max-w-4xl mx-auto mb-8">
      <div className="bg-slate-800/80 border border-pink-400/30 rounded-xl p-6 text-center backdrop-blur-sm">
    ...
    <div className="min-h-screen sm:pt-20 bg-gradient-to-br from-slate-900 via-gray-900 to-zinc-900">
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        <div className="text-center mb-12">
          <h1 className="sm:text-5xl text-3xl font-bold text-white mb-4 bg-gradient-to-r from-pink-400 via-purple-400 to-blue-400 bg-clip-text text-transparent">
            TikTok Downloader
          </h1>
          ...
          <div className="flex justify-center items-center gap-2 mt-4">
            ...
            <span className="text-pink-300 font-semibold">Watermark-Free Downloads</span>
            <Star className="h-5 w-5 text-pink-400" />
          </div>
        </div>
                  ...
                  {downloadMode === 'video' ? (
                    <div>
                      <div className="flex items-center gap-2 mb-4">
                        <FileVideo className="h-5 w-5 text-pink-400" />
                      ...
                      {videoInfo.formats?.video_formats && videoInfo.formats.video_formats.length > 0 ? (
                            ...
                            return (
                              <div key={index} className="bg-slate-700/50 rounded-xl p-4 border border-pink-500/20">
                                ...
                                <button
                                  ...
                                  className="w-full bg-gradient-to-r from-pink-500 to-purple-500 hover:from-pink-600 hover:to-purple-600 text-white font-medium py-2 px-4 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                >
                        ...
                        <div className="text-center py-8">
                          <p className="text-gray-400 text-lg">No video formats available for this TikTok.</p>
                        </div>
                      )}
                    </div>
                  ) : (
                    <div>
                      ...
                      {videoInfo.formats?.audio_formats && videoInfo.formats.audio_formats.length > 0 ? (
                            ...
                            return (
                              <div key={index} className="bg-slate-700/50 rounded-xl p-4 border border-purple-500/20">
                                ...
                                <button
                                  onClick={() => handleDownload(format)}
                                  disabled={!!progress && !['error', 'downloaded'].includes(progress.status)}
                                  className="w-full bg-gradient-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 text-white font-medium py-2 px-4 rounded-lg transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                >
                                  <Download className="h-4 w-4" />
                                  {getButtonText(progress)}
                                </button>
                        ...
                        <div className="text-center py-8">
                          <p className="text-gray-400 text-lg">No audio formats available for this TikTok.</p>
                        </div>
                      )}
                    </div>
                  )}
        ...
        <div className="text-center mt-12 text-gray-400">
          <p>© 2025 TikTok Downloader. Download videos without watermark in HD quality.</p>
          <p className="text-sm mt-2">Supports all TikTok video formats • Fast downloads • No registration required</p>
        </div>
      </div>
    </div>
  );
